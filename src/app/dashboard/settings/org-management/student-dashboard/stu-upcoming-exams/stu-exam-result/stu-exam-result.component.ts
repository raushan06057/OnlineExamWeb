...
export class StuExamResultComponent implements OnInit {
  ...
  bindResult() {
    this.studentService
      .getStudentExamResultById(this.examId)
      .subscribe((res) => {
        debugger;
        this.resultModel = res.data;
        this.feedback =
          res?.chatResponse?.messages?.[0]?.contents?.[0]?.text ??
          'No text available';
      });
  }
  ...
  displayGraph() {
    this.studentService
      .getStudentExamResultByIdGraph(this.examId)
      .subscribe((res) => {
        debugger;
        const rawHtml =
          res?.data?.messages?.[0]?.contents?.[0]?.text ?? 'No text available';
        this.feedbackGraphHtml =
